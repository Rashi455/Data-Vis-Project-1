...
d3.select('.legend-btn[category="All"]')
  .style('opacity', 1)
  .style('font-weight', 'bold');

d3.selectAll('.legend-btn').on('click', function() {
  selectedCategory = d3.select(this).attr('category');
  
  // Filter countyData based on the selected category
  if (selectedCategory === "All") {
    lineFilter = countyData; // Show all counties
  } else {
    lineFilter = countyData.filter(d => d.urban_rural_status === selectedCategory);
  }
  ...
  d3.selectAll('.legend-btn')
    .style('opacity', 0.5); // fade out all buttons

  d3.select(this)
    .style('opacity', 1) // highlight the clicked button
    .style('font-weight', 'bold');
});
